...
**Prevention:** Implemented `fetchUrl`, `fetchJson`, and `getJSDOM` utilities in `src/utils/scraperUtils.js` that enforce a default 30s timeout via `AbortController` and set a consistent User-Agent. All scrapers should use these utilities instead of raw network calls.
